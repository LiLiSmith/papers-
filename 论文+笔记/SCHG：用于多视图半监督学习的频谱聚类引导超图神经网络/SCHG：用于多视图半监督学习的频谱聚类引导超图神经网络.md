# SCHG：用于多视图半监督学习的频谱聚类引导超图神经网络

## 1.读Abstract

### ①问题？

多统的超图构建方法通常只考虑样本之间的局部相关性，可能会忽略数据集更广泛上下文中存在的依赖关系。

### ②方法&创新点？

在本文中，我们提出了一种新颖的多视图高阶相关建模方法，其中超边的连通性通过聚类确定，并通过超图神经网络集成来自每个视图的互补信息。受谱图理论揭示的图可整除性的启发，所提出的方法可以很好地捕获数据中的全局高阶相关性并发现潜在的流形。为了评估超图建模的有效性，我们对多视图半监督节点分类任务进行了综合评估。

### ③达到的效果？

实验表明，与当前最先进的算法和跨八个数据集的通用超图学习相比，所提出的方法取得了更优异的性能。
